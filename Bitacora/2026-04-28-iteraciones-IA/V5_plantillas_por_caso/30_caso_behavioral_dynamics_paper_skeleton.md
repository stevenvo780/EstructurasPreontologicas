...
Aplicamos el aparato Effective Dependence Index (EDI) a 30 — behavioral dynamics para
...
tests adversariales) reportamos EDI = 0.26216558939208157, p_block = 0.9783333333333334, CI 95% = [0.24938936485515273, 0.2797971776507399].
La sonda secundaria con motivación teórica independiente (sincronizacion_de_fases) produce
|Δ EDI| = 0.12229061628463533. Pre-registro criptográfico bajo hash 242d33bce16712b8
(commit a24f6dd14049). Datos: Sintético generado por sistema completo Fajen-Warren.
...
**Fuente:** Sintético generado por sistema completo Fajen-Warren
...
**Sonda secundaria:** kuramoto_phase_locking

**Motivación teórica:** sincronizacion_de_fases
...
**Pre-registro:** SETUP_HASH.json con aggregate_hash = `242d33bce16712b8`
...
- **EDI puntual:** 0.26216558939208157
- **p-value naive:** 0.044044044044044044
- **p-value block bootstrap (V5.1):** 0.9783333333333334
- **CI bootstrap 95%:** [0.24938936485515273, 0.2797971776507399]

### 3.2. Convergencia con sonda secundaria

|Δ EDI| inter-paradigma = 0.12229061628463533
...
- Sintético; elevación con datos VENLab/WALK-MS pendiente
...
**Hash del setup pre-ejecución:** `242d33bce16712b8...`
...
```bash
python3 scripts/freeze_setup.py --verify 30_caso_behavioral_dynamics
```
