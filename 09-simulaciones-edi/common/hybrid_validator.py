...
def _effective_cpu_count(default: int = 4) -> int:
    """
    CPU efectiva para paralelismo interno por proceso.

    Prioridad:
      1) HYPER_CPU_THREADS / HYPER_N_JOBS (override explícito).
      2) afinidad del proceso (cpuset/cgroups).
      3) os.cpu_count().
    """
    env_override = os.environ.get("HYPER_CPU_THREADS") or os.environ.get("HYPER_N_JOBS")
    if env_override:
        try:
...
def _parallel_jobs(n_items: int, has_gpu: bool = False, batch_enabled: bool = False) -> int:
    """
    Número de procesos para joblib.

    Regla crítica:
    - Si hay GPU activa PERO no hay batch GPU disponible, usar 1 proceso.
      Multiproceso en ese escenario crea múltiples contextos CUDA y dispara
      tráfico host↔device con baja ocupación útil (thrash PCIe).
    """
    if n_items <= 1:
        return 1
    if has_gpu and not batch_enabled:
        return 1
    return max(1, min(int(n_items), _effective_cpu_count()))
...
def calibrate_abm(obs_train, base_params, steps, simulate_abm_fn,
    ...
    def _score_batch(preds_matrix):
        """
        Score batch: preds_matrix (B, T), obs_arr (T,).

        Si `preds_matrix` viene en CuPy, se puntúa en GPU y solo se copian a
        CPU los vectores (B,) de score/error para minimizar tráfico por bus.
        """
        try:
            import cupy as cp
            is_cupy = isinstance(preds_matrix, cp.ndarray)
        except Exception:
            cp = None
            is_cupy = False
        ...
        if is_cupy:
            ...
            y = cp.asarray(obs_arr, dtype=cp.float64)
            diff = X - y[None, :]
            errs = cp.sqrt(cp.mean(diff ** 2, axis=1))
            ...
            corrs = cp.where(den > 1e-15, num / den, 0.0)
            penalties = cp.clip(2.0 - corrs, 0.5, None)
            scores = errs * penalties
            return cp.asnumpy(scores), cp.asnumpy(errs)

        # CPU / NumPy path
        X = np.asarray(preds_matrix[:, :n_obs], dtype=np.float64)
        diff = X - obs_arr[None, :]
        errs = np.sqrt(np.mean(diff ** 2, axis=1))
        X_m = X - X.mean(axis=1, keepdims=True)
        y_m = obs_arr - obs_arr.mean()
        X_std = X.std(axis=1)
        y_std = obs_arr.std()
        num = (X_m * y_m[None, :]).sum(axis=1)
        den = X_std * y_std * n_obs
        corrs = np.zeros(X.shape[0], dtype=np.float64)
        valid = den > 1e-15
    ...
    # ─── Validar compatibilidad de batch GPU con el simulate_abm del caso ─────
    # Evita calibrar modelos de caso personalizados con el kernel genérico
    # de abm_core_gpu, lo que puede producir parámetros incorrectos.
    if _batch_fn is not None and grid_combos:
        try:
            probe_variant = grid_combos[0]
            probe_bp = dict(base_params)
            probe_bp["assimilation_strength"] = 0.0
            probe_bp["assimilation_series"] = None
            probe_bp["_store_grid"] = False

            probe_params = dict(probe_bp)
            probe_params.update(probe_variant)
            sim_probe = simulate_abm_fn(probe_params, steps, seed=seed)
            key_probe = _get_series_key(sim_probe)
            probe_series = np.asarray(sim_probe[key_probe][:n_obs], dtype=np.float64)
            if probe_series.ndim == 3 and obs_arr.ndim == 1:
                probe_series = probe_series.mean(axis=(1, 2))

            try:
                batch_probe = _batch_fn(
                    probe_bp, [probe_variant], steps, seed=seed,
                    series_key=series_key, init_range=float(probe_bp.get("init_range", 0.5)),
                    return_numpy=True,
                )
            except TypeError:
                batch_probe = _batch_fn(
            ...
            batch_series = np.asarray(batch_probe[0, :n_obs], dtype=np.float64)
            if batch_series.ndim == 3 and obs_arr.ndim == 1:
                batch_series = batch_series.mean(axis=(1, 2))

            if len(probe_series) != len(batch_series):
                raise RuntimeError("batch probe size mismatch")

            probe_rmse = float(np.sqrt(np.mean((probe_series - batch_series) ** 2)))
            probe_scale = float(np.std(probe_series))
            # Tolerancia relativa suave (2% de escala) + piso absoluto.
            tol = max(1e-4, 0.02 * max(probe_scale, 1e-6))
            if (not np.isfinite(probe_rmse)) or (probe_rmse > tol):
                print(
                    "[calibrate] GPU batch desactivado: kernel batch no coincide "
                    f"con simulate_abm del caso (rmse_probe={probe_rmse:.4g}, tol={tol:.4g})",
                    file=sys.stderr,
                )
                _batch_fn = None
        except Exception as _probe_err:
            print(
                "[calibrate] GPU batch desactivado: chequeo de compatibilidad falló "
                f"({type(_probe_err).__name__})",
                file=sys.stderr,
            )
            _batch_fn = None
    ...
    if _batch_fn is not None:
        ...
        # Una sola llamada GPU → (B, steps) matrix (en device cuando sea posible)
        try:
            preds_matrix = _batch_fn(
                bp, grid_combos, steps, seed=seed,
        ...
        n_jobs = _parallel_jobs(len(grid_combos), has_gpu=_has_gpu, batch_enabled=False)
    ...
    if _batch_fn is not None:
        ...
        n_jobs = _parallel_jobs(refine_per_point, has_gpu=_has_gpu, batch_enabled=False)
    ...
    for rank in range(top_k):
        ...
        while i < refine_per_point:
            ...
            if _batch_fn is not None:
                ...
                try:
                    preds = _batch_fn(
                        bp, batch_candidates, steps, seed=seed,
                        series_key=series_key, init_range=float(bp.get("init_range", 0.5)),
                        return_numpy=False,
                    )
                except TypeError:
...
def evaluate_c2(base_params, eval_params, steps, val_start,
    ...
    if _batch_fn is not None:
        ...
        n_jobs = _parallel_jobs(n_pert, has_gpu=_has_gpu, batch_enabled=False)
        results = Parallel(n_jobs=n_jobs, backend="loky")(
...
def evaluate_c5(base_params, eval_params, steps, val_start,
    ...
    if _batch_fn is not None:
        ...
        n_jobs = _parallel_jobs(n_runs, has_gpu=_has_gpu, batch_enabled=False)
...
def _get_git_info():
    def _parse_dirty(raw):
        ...
        if v in ("1", "true", "yes", "y", "dirty"):
            return True
        if v in ("0", "false", "no", "n", "clean"):
            return False
        return None

    # Prioridad 1: metadata inyectada por runner (host), robusta en Docker sin .git.
    env_commit = (os.environ.get("HYPER_GIT_COMMIT") or os.environ.get("GIT_COMMIT") or "").strip()
    env_dirty = _parse_dirty(os.environ.get("HYPER_GIT_DIRTY"))
    if env_commit:
        return {"commit": env_commit, "dirty": env_dirty}

    # Prioridad 2: intentar resolver git localmente (host o contenedor con .git montado).
    candidates = []
    for p in (
        os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")),       # .../Simulaciones
        os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..")), # .../repos o raíz
