    ...
    for t in range(steps):
        ...
        if assim_series is not None and t < len(assim_series):
            ...
            if target is not None:
                # Igual que CPU: nudging contra el promedio ACTUAL del grid
                # (después del update/clip/perturbación del paso t).
        ...
        if macro_mode == "dynamic":
            ...
            # Igual que CPU: en modo "mean" se reporta el promedio del grid
            # DESPUÉS del update del paso t (no el valor previo).
...
def simulate_abm_batch(
    ...
    return_numpy : bool
        Si True, devuelve np.ndarray (CPU). Si False y hay GPU, devuelve
        tensor en device (CuPy) para evitar copias host↔device intermedias.
    ...
    if B == 0:
        ...
        return _xp.empty((0, steps), dtype=_xp.float64)
    ...
    if B > effective_batch:
        ...
        return _xp.concatenate(all_results, axis=0)
    ...
    try:
        return _simulate_abm_batch_kernel(
            ...
            _xp, B, n, return_numpy=return_numpy,
        )
        ...
        if "OutOfMemory" in type(oom_err).__name__ or "out of memory" in str(oom_err).lower():
            ...
            r1 = simulate_abm_batch(
                base_params, param_variants[:half], steps, seed, series_key, init_range,
                max_batch=0, return_numpy=return_numpy
            )
            cp.get_default_memory_pool().free_all_blocks()
            r2 = simulate_abm_batch(
                base_params, param_variants[half:], steps, seed, series_key, init_range,
                max_batch=0, return_numpy=return_numpy
            )
            if return_numpy:
                return np.concatenate([r1, r2], axis=0)
            return _xp.concatenate([r1, r2], axis=0)
        raise


def _simulate_abm_batch_kernel(
    base_params, param_variants, steps, seed, series_key, init_range,
    _xp, B, n, return_numpy: bool = True,
):
    ...
    for t in range(steps):
        ...
        # Noise batch: reproducir semántica CPU de seed fijo por candidato.
        # En CPU, cada candidato se evalúa con el MISMO seed, por lo que
        # comparte la misma realización base de ruido (solo cambia su escala).
        noise_base = rng.uniform(-1.0, 1.0, (n, n))
        if use_hetero:
            noise = noise_base[None, :, :] * noise_map[None, :, :]
        elif _noise_varies:
            noise = noise_base[None, :, :] * noise_arr[:, None, None]
        else:
            noise = noise_base[None, :, :] * noise_amp
