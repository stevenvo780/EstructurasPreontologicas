...
Aplicamos el aparato Effective Dependence Index (EDI) a 24 — microplasticos para
...
tests adversariales) reportamos EDI = 0.7818771547395869, p_block = 0.0003333333333333333, CI 95% = [0.6457820421229035, 0.870992022426998].
La sonda secundaria con motivación teórica independiente (crecimiento_asimetrico) produce
|Δ EDI| = 0.9689673831919933. Pre-registro criptográfico bajo hash 96623651410fb65c
(commit a24f6dd14049). Datos: Jambeck 2015 et al. + OWID.
...
**Fuente:** Jambeck 2015 et al. + OWID
...
**Sonda secundaria:** gompertz_pollutant

**Motivación teórica:** crecimiento_asimetrico
...
**Pre-registro:** SETUP_HASH.json con aggregate_hash = `96623651410fb65c`
...
- **EDI puntual:** 0.7818771547395869
- **p-value naive:** 0.0
- **p-value block bootstrap (V5.1):** 0.0003333333333333333
- **CI bootstrap 95%:** [0.6457820421229035, 0.870992022426998]

### 3.2. Convergencia con sonda secundaria

|Δ EDI| inter-paradigma = 0.9689673831919933
...
- Jambeck, J. R. et al. (2015). *Science* 347(6223).
...
**Hash del setup pre-ejecución:** `96623651410fb65c...`
...
```bash
python3 scripts/freeze_setup.py --verify 24_caso_microplasticos
```
