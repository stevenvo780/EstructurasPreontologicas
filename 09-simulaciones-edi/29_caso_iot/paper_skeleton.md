...
Aplicamos el aparato Effective Dependence Index (EDI) a 29 — iot para
...
tests adversariales) reportamos EDI = -0.8760101554802692, p_block = 0.0003333333333333333, CI 95% = [-1.2158519908237377, -0.7112972885147779].
La sonda secundaria con motivación teórica independiente (fisher_pry_substitution) produce
|Δ EDI| = 0.8801388044576907. Pre-registro criptográfico bajo hash 805e71ae59f84c9e
(commit a24f6dd14049). Datos: Statista IoT proxy + ITU.
...
**Sonda secundaria:** logistic_substitution

**Motivación teórica:** fisher_pry_substitution
...
**Pre-registro:** SETUP_HASH.json con aggregate_hash = `805e71ae59f84c9e`
...
- **EDI puntual:** -0.8760101554802692
- **p-value naive:** 1.0
- **p-value block bootstrap (V5.1):** 0.0003333333333333333
- **CI bootstrap 95%:** [-1.2158519908237377, -0.7112972885147779]

### 3.2. Convergencia con sonda secundaria

|Δ EDI| inter-paradigma = 0.8801388044576907
...
**Hash del setup pre-ejecución:** `805e71ae59f84c9e...`
...
```bash
python3 scripts/freeze_setup.py --verify 29_caso_iot
```
