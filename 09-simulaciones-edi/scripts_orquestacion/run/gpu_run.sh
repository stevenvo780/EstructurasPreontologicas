...
# ── USO ───────────────────────────────────────────────────────────────────────
#
#   # Todo de golpe (29 casos con distribución multi-GPU estable)
#   ./gpu_run.sh
#
...
#   # Ajustar concurrencia por GPU (default estable: 1 worker por GPU)
#   ./gpu_run.sh --workers-per-gpu 2
#
# ── FLAGS ─────────────────────────────────────────────────────────────────────
#
...
#   --workers-per-gpu N  Workers por GPU (default: 1, estable para PCIe)
#   --max-threads-per-proc N  Tope de hilos CPU por proceso validate.py (default: 12)
...
# Importante: el contenedor puede no tener .git montado.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --max-threads-per-proc) MAX_THREADS_PER_PROC="$2"; shift 2 ;;
...
resolve_case_grid() {
    local case_name="$1"
    local g="${CASE_GRID[$case_name]:-}"
    if [[ -n "$g" && "$g" =~ ^[0-9]+$ && "$g" -ge 1 ]]; then
        echo "$g"
        return
    fi
    local cfg="$SIM_DIR/$case_name/case_config.json"
    if [[ -f "$cfg" ]]; then
        local parsed
        parsed=$(grep -oE '"grid_size"[[:space:]]*:[[:space:]]*[0-9]+' "$cfg" | head -1 | grep -oE '[0-9]+' || true)
        if [[ -n "$parsed" && "$parsed" =~ ^[0-9]+$ && "$parsed" -ge 1 ]]; then
            echo "$parsed"
            return
        fi
    fi
...
run_batch() {
    ...
    # Calcular workers por GPU: por defecto modo estable (1 worker/GPU).
    # Más workers por GPU sólo bajo ajuste explícito (--workers-per-gpu),
    # porque multiproceso CUDA aumenta riesgo de thrash PCIe/context-switch.
    #
    ...
    local target_per_gpu="$WORKERS_PER_GPU"
    [[ $target_per_gpu -lt 1 ]] && target_per_gpu=1
    
    for i in "${!AVAILABLE_GPUS[@]}"; do
        local gid="${AVAILABLE_GPUS[$i]}"
        local free=${GPU_FREE_MB[$i]}
        local max_grid="${GPU_MAX_GRID[$gid]:-1}"
        local assigned_cases="${GPU_CASES[$gid]:-}"
        local n_assigned=0
        if [[ -n "$assigned_cases" ]]; then
            n_assigned=$(wc -w <<< "$assigned_cases")
        fi

        if [[ $n_assigned -eq 0 ]]; then
            echo "    GPU ${gid}: 0 casos asignados, 0 workers"
            workers_per_gpu+=(0)
            continue
        fi
        ...
        local nw=$target_per_gpu
        
        # Cap por VRAM: nunca más workers de lo que la GPU soporta
        [[ $nw -gt $vram_max_workers ]] && nw=$vram_max_workers
        # Cap por carga real: no más workers que casos asignados a esta GPU
        [[ $nw -gt $n_assigned ]] && nw=$n_assigned
...
# Thread budget por proceso para evitar over-subscription CPU+RAM+bus.
TOTAL_CORES=\$(nproc 2>/dev/null || echo 8)
THREADS_PER_PROC=\$(( TOTAL_CORES / (N_WORKERS > 0 ? N_WORKERS : 1) ))
[[ \$THREADS_PER_PROC -lt 1 ]] && THREADS_PER_PROC=1
[[ \$THREADS_PER_PROC -gt ${MAX_THREADS_PER_PROC} ]] && THREADS_PER_PROC=${MAX_THREADS_PER_PROC}
...
gpu_worker() {
    ...
    while true; do
        ...
        local GRID_CASE=\${CASE_GRID_MAP[\$caso]:-1}
...
for gpu in ${AVAILABLE_GPUS[*]}; do
    cnt=\$(grep -c \"GPU\${gpu}\" \"\$RESULT_FILE\" 2>/dev/null || true)
    [[ -z \"\$cnt\" ]] && cnt=0
...
echo "║  W/GPU:    ${WORKERS_PER_GPU} (ajustable con --workers-per-gpu)"
echo "║  Thr/proc: max ${MAX_THREADS_PER_PROC} (ajustable con --max-threads-per-proc)"
...
if [[ $STEP_BY_STEP -eq 1 ]]; then
    ...
    n_sbs_gpus=${#sbs_gpus[@]}
    SBS_TOTAL_CORES=$(nproc 2>/dev/null || echo 8)
    SBS_THREADS_PER_PROC=$(( SBS_TOTAL_CORES / (n_sbs_gpus > 0 ? n_sbs_gpus : 1) ))
    [[ $SBS_THREADS_PER_PROC -lt 1 ]] && SBS_THREADS_PER_PROC=1
    [[ $SBS_THREADS_PER_PROC -gt ${MAX_THREADS_PER_PROC} ]] && SBS_THREADS_PER_PROC=${MAX_THREADS_PER_PROC}
    ...
    echo "  Threads/proceso: ${SBS_THREADS_PER_PROC} (HYPER_CPU_THREADS)"
    ...
    _gpu_loop() {
        ...
        while true; do
            ...
            local LOG="${LOG_DIR}/${caso}.log"
            local GRID_CASE
            GRID_CASE=$(resolve_case_grid "$caso")
...
export HYPER_GIT_COMMIT='${HOST_GIT_COMMIT}'
export HYPER_GIT_DIRTY='${HOST_GIT_DIRTY}'
export HYPER_CPU_THREADS=${SBS_THREADS_PER_PROC}
export HYPER_N_JOBS=${SBS_THREADS_PER_PROC}
export OMP_NUM_THREADS=${SBS_THREADS_PER_PROC}
export MKL_NUM_THREADS=${SBS_THREADS_PER_PROC}
export OPENBLAS_NUM_THREADS=${SBS_THREADS_PER_PROC}
export NUMEXPR_NUM_THREADS=${SBS_THREADS_PER_PROC}
mkdir -p ${LOG_DIR}
