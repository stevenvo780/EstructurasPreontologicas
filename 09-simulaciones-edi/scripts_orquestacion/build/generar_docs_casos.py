...
## Auditoría
- `repos/scripts/audit/auditar_simulaciones.py` verifica todos los 29 casos.
- `repos/scripts/build/evaluar_simulaciones.py` genera tablas resumen.
...
def _parse_cases(raw):
    ...
    for token in raw.split(","):
        ...
        try:
            ...
            raise SystemExit(f"Valor inválido en --cases: {token!r}. Usa enteros CSV, p.ej. 19,20,21")
        if n not in CASES:
            raise SystemExit(f"Caso fuera de rango en --cases: {n}. Rango válido: 19-29.")
        selected.append(n)
    if not selected:
        raise SystemExit("No se especificaron casos válidos en --cases.")
    return sorted(set(selected))


def main():
    parser = argparse.ArgumentParser(
        description="Genera docs/ estándar (arquitectura, protocolo, indicadores, validación, reproducibilidad) para casos 19-29."
    )
    parser.add_argument(
        "--cases",
        help="CSV de casos a generar (ej: 19,20,24). Por defecto genera 19-29.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Solo muestra archivos a generar; no escribe en disco.",
    )
    args = parser.parse_args()

    selected_cases = _parse_cases(args.cases)
    generated = 0
    for num in selected_cases:
        info = CASES[num]
        ...
        if args.dry_run:
            print(f"OK (dry-run): {num:02d}_caso_{info['name']}: {len(files)} docs listados")
        else:
            print(f"✅ {num:02d}_caso_{info['name']}: {len(files)} docs generados")

    if args.dry_run:
        print(f"\nOK (dry-run): {generated} archivos listados para {len(selected_cases)} casos")
    else:
        print(f"\n✅ Total: {generated} archivos generados para {len(selected_cases)} casos")
