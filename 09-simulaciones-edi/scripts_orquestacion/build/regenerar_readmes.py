...
def generate_readme(caso_dir, m):
    ...
    lines.append('')
    lines.append('<!-- AUTO:RESULTS:START -->')
...
def _match_case(caso: str, case_filter: str) -> bool:
    if not case_filter:
        return True
    return case_filter.lower() in caso.lower()


def main():
    parser = argparse.ArgumentParser(
        description="Regenera README.md de TesisDesarrollo/02_Modelado_Simulacion desde outputs/metrics.json"
    )
    parser.add_argument(
        "--case",
        help="Filtro parcial case-insensitive (ej: clima, 06, falsacion).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Solo muestra qué casos/archivos se regenerarían, sin escribir.",
    )
    args = parser.parse_args()

    cases = sorted([d for d in os.listdir(SIM_BASE)
                    if '_caso_' in d and os.path.isdir(os.path.join(SIM_BASE, d)) and _match_case(d, args.case)])
    ...
    if args.dry_run:
        print(f'\nOK (dry-run): {updated} READMEs listados en TesisDesarrollo/02_Modelado_Simulacion/')
    else:
