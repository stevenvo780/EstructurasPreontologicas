...
def _select_cases(pattern=None, cases_raw=None):
    cases = sorted(CASES_ROOT.glob('*_caso_*'))
    if pattern:
    ...
    if cases_raw:
        ...
        for tok in tokens:
            if tok.isdigit():
                ...
                filtered.extend([c for c in cases if c.name.startswith(prefix)])
            else:
                filtered.extend([c for c in cases if tok in c.name])
        cases = sorted(set(filtered))
    return cases


def main():
    parser = argparse.ArgumentParser(description="Auditoria documental y metrica (solo lectura)")
    ...
    parser.add_argument("--output", default=str(OUTPUT), help="Ruta de salida Markdown")
    parser.add_argument("--stdout", action="store_true", help="Imprimir a stdout en vez de escribir archivo")
    ...
    for case_dir in _select_cases(args.pattern, args.cases):
    ...
    if args.stdout:
        ...
        print(f"Auditoría escrita en {args.output}")
