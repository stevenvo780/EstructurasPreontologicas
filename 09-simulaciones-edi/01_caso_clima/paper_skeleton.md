...
Aplicamos el aparato Effective Dependence Index (EDI) a 01 — clima para
...
tests adversariales) reportamos EDI = 0.011072299899552383, p_block = 0.0003333333333333333, CI 95% = [-0.0032760206049711514, 0.028363288615172646].
La sonda secundaria con motivación teórica independiente (fisica_oscilatoria) produce
|Δ EDI| = 1.0107868710425527. Pre-registro criptográfico bajo hash 90f533060bbaec34
(commit a24f6dd14049). Datos: Meteostat / NOAA.
...
**Sonda secundaria:** stochastic_harmonic_oscillator

**Motivación teórica:** fisica_oscilatoria
...
**Pre-registro:** SETUP_HASH.json con aggregate_hash = `90f533060bbaec34`
...
- **EDI puntual:** 0.011072299899552383
- **p-value naive:** 0.998998998998999
- **p-value block bootstrap (V5.1):** 0.0003333333333333333
- **CI bootstrap 95%:** [-0.0032760206049711514, 0.028363288615172646]

### 3.2. Convergencia con sonda secundaria

|Δ EDI| inter-paradigma = 1.0107868710425527
...
**Hash del setup pre-ejecución:** `90f533060bbaec34...`
...
```bash
python3 scripts/freeze_setup.py --verify 01_caso_clima
```
