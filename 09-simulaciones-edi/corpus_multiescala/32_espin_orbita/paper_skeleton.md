...
Aplicamos el aparato Effective Dependence Index (EDI) a 32 espin orbita para
...
tests adversariales) reportamos EDI = 0.8250879834716865, p_block = 0.0003333333333333333, CI 95% = [0.7970136996952593, 0.8508715796440832].
La sonda secundaria con motivación teórica independiente (fisica_estadistica_estocastica) produce
|Δ EDI| = 1.8250374995617422. Pre-registro criptográfico bajo hash 840bc61ba5876851
(commit a24f6dd14049). Datos: Sintético Bloch — parámetros NV-center.
...
**Fuente:** Sintético Bloch — parámetros NV-center
...
**Sonda secundaria:** langevin_dynamics

**Motivación teórica:** fisica_estadistica_estocastica
...
**Pre-registro:** SETUP_HASH.json con aggregate_hash = `840bc61ba5876851`
...
- **EDI puntual:** 0.8250879834716865
- **p-value naive:** 0.0
- **p-value block bootstrap (V5.1):** 0.0003333333333333333
- **CI bootstrap 95%:** [0.7970136996952593, 0.8508715796440832]

### 3.2. Convergencia con sonda secundaria

|Δ EDI| inter-paradigma = 1.8250374995617422
...
**Hash del setup pre-ejecución:** `840bc61ba5876851...`
...
```bash
python3 scripts/freeze_setup.py --verify 32_espin_orbita
```
