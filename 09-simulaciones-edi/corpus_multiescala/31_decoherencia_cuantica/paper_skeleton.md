...
Aplicamos el aparato Effective Dependence Index (EDI) a 31 decoherencia cuantica para
...
tests adversariales) reportamos EDI = 0.913177796635811, p_block = 0.0003333333333333333, CI 95% = [0.8854121570965395, 0.9315969912337876].
La sonda secundaria con motivación teórica independiente (decay_exponencial) produce
|Δ EDI| = 0.32573916122415225. Pre-registro criptográfico bajo hash 5b8af6e2aaa8a57d
(commit a24f6dd14049). Datos: Sintético Lindblad — parámetros transmón NIST/IBM.
...
**Fuente:** Sintético Lindblad — parámetros transmón NIST/IBM
...
**Sonda secundaria:** exponential_relaxation

**Motivación teórica:** decay_exponencial
...
**Pre-registro:** SETUP_HASH.json con aggregate_hash = `5b8af6e2aaa8a57d`
...
- **EDI puntual:** 0.913177796635811
- **p-value naive:** 0.0
- **p-value block bootstrap (V5.1):** 0.0003333333333333333
- **CI bootstrap 95%:** [0.8854121570965395, 0.9315969912337876]

### 3.2. Convergencia con sonda secundaria

|Δ EDI| inter-paradigma = 0.32573916122415225
...
**Hash del setup pre-ejecución:** `5b8af6e2aaa8a57d...`
...
```bash
python3 scripts/freeze_setup.py --verify 31_decoherencia_cuantica
```
