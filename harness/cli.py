...
La capa LLM (sub-agentes, orquestación filosófica) vive 100% en Claude Code:
ver `.claude/agents/` y skills `/harness-pass`, `/tesis-pass`. Este CLI no
invoca modelos: produce JSON y reportes deterministas que sub-agentes leen.
"""
...
def cmd_init(args):
    ...
    print(f"[init] skills: ver .claude/skills/")
    save_state(state)
